...
# --- Game Launch Logic ---
def launch_game(version_id, account, ram_mb=1024, java_path="java", game_dir=None, server_ip=None, port=None, status_callback=None, use_rosetta=False):
...
class M1LauncherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("M1 Minecraft Launcher v1.2")
        ...
        ttk.Label(m1_frame, text=f"Detected CPU architecture: {arch_text}").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        ...
        ttk.Label(m1_frame, text=f"Rosetta 2: {rosetta_text}").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        ...
        ttk.Radiobutton(acct_frame, text="Microsoft (Demo)", variable=self.acct_type_var, value="microsoft", state="disabled").grid(row=0, column=2, sticky="w", padx=5)
    ...
    def on_launch(self):
        ...
        launch_thread = threading.Thread(
            ...
            args=(version_to_process, is_modpack, selected_account, ram_val, java_path_val, server_ip_val, port_val, use_rosetta),
            daemon=True
        )
        launch_thread.start()

    def _launch_task(self, item_to_launch, is_modpack, account, ram, java, server, port, use_rosetta):
